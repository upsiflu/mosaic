module Article exposing 
    ( Article, Format, Draft, Caret, Msg, Appearance
    -- create
    , singleton
    -- from Editor, via Mosaic:
    , update
    -- from Mosaic:
    {-, got-}
    , view
...
import Html exposing (Html, div, strong, text, span, pre)
import Html.Attributes exposing (class, attribute)
import Html.Events exposing (on, onDoubleClick, onBlur)
import Ui exposing (..)
...
view appearance article =
    let decodeOn event decoder message =
            decoder
            |> Decode.map message
            |> on event
        
        toolbar_placeholder = div [ class "toolbar "] [] 

    in case appearance of
        Normal ->
            Ui.Static
            [ toolbar_placeholder
...
viewToolbar on_format ( Caret caret ) =
    let
        ...
        decideState on off =
            is_in caret >> \activated->
                { toggle = on_format 
                    ( if activated then off else on )
                , is_on = activated }
