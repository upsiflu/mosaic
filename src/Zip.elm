module Zip exposing 
  (Path(..), IntPath, Wing(..), Zip, focus
    , foldl_left_side, foldl_right_side
    , foldl_homogenous, fold_homogenous, foldr_homogenous
    , foldr_left_side, foldr_right_side
    , foldl, fold, foldr
    , insert_left, insert_right, left, length
    , map_periphery, map_focus
    , indexed_map_periphery, indexed_map_focus
    , path_to_string, path_to_int
    , int_to_path
    , reverse, right, singleton
    , to_string, walk)
...
foldl_left_side acc primer (Zip l _ _) = List.foldl acc primer l

foldl_right_side : ( a -> b -> b ) -> b -> Zip c a -> b
foldl_right_side acc primer (Zip _ _ r) = List.foldl acc primer r

foldr_left_side : ( a -> b -> b ) -> b -> Zip c a -> b
foldr_left_side acc primer (Zip l _ _) = List.foldr acc primer l

foldr_right_side : ( a -> b -> b ) -> b -> Zip c a -> b
foldr_right_side acc primer (Zip _ _ r) = List.foldr acc primer r
...
int_to_path i =
    if      ( i < 0 ) then L <| int_to_path ( i+1 )
    else if ( i > 0 ) then R <| int_to_path ( i-1 )
    else    Here
